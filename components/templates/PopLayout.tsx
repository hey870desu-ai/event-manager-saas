        ...
        <div className="max-w-xl w-full text-center space-y-8 animate-bounce-in bg-white p-10 rounded-3xl border-4 border-slate-900 shadow-[8px_8px_0px_0px_rgba(15,23,42,1)]">
          ...
          {(event.preSurveyFields?.length > 0 || event.preSurveyUrl) && (
            <div className="bg-purple-50 border-4 border-purple-300 rounded-2xl p-4 text-center">
              ...
              <a href={event.preSurveyFields?.length > 0 ? `/t/${tenantId}/e/${eventId}/pre-survey?rid=${reservationId}` : event.preSurveyUrl} target="_blank" rel="noopener noreferrer" className="inline-block px-6 py-2.5 bg-purple-600 hover:bg-purple-500 text-white font-black rounded-xl transition-colors text-sm">アンケートに回答する</a>
            </div>
          )}
