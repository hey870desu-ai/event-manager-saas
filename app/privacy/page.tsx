...
function PrivacyContent() {
  ...
  const orgName = tenant?.orgName || tenant?.name || "絆太郎 Event Manager";
  const companyName = tenant?.legalCompanyName || orgName;
  const representative = tenant?.representative || "";
  const address = tenant?.address || "";
  const email = tenant?.legalEmail || tenant?.contactEmail || "";
        ...
        <div className="bg-white rounded-3xl shadow-sm border border-slate-200 overflow-hidden">
          ...
          <div className="p-8 md:p-12 space-y-10 text-sm md:text-base leading-relaxed">
            ...
            <section>
              ...
              <p>{companyName}（以下「当社」）は，ユーザーが本サービスを利用する際、氏名，会社名，部署名，メールアドレス，電話番号などの個人情報を取得することがあります。また、ユーザーが主催するイベントの参加者が入力した情報についても、本サービスの提供・運営および主催者への提供のために取得します。</p>
            </section>
            ...
            <section>
              ...
              <strong>・当社によるイベント運営、受付管理、および当社が実施するサービス・イベントに関する案内のため</strong></p>
            </section>

            <section>
              <h2 className="text-lg font-bold border-l-4 border-emerald-600 pl-3 mb-4 text-slate-900">第4条（第三者提供の制限）</h2>
            ...
            <section>
              <h2 className="text-lg font-bold border-l-4 border-emerald-600 pl-3 mb-4 text-slate-900">第5条（安全管理措置）</h2>
            ...
            <section>
              <h2 className="text-lg font-bold border-l-4 border-emerald-600 pl-3 mb-4 text-slate-900">第6条（個人情報の開示・訂正・削除）</h2>
              <p>ユーザーおよび参加者は，当社の保有する自己の個人情報の訂正，追加または削除を請求することができます。当社は，これらの請求を受けた場合，遅滞なく調査を行い，適切に対応いたします。</p>
            </section>

            <section>
              <h2 className="text-lg font-bold border-l-4 border-emerald-600 pl-3 mb-4 text-slate-900">第7条（お問い合わせ窓口）</h2>
              <p>本ポリシーに関するお問い合わせは，下記の窓口までお願いいたします。</p>
              <div className="mt-4 bg-slate-50 rounded-lg p-4 border border-slate-200 space-y-1 text-sm">
                {address && <p>住所：{address}</p>}
                <p>社名：{companyName}</p>
                {representative && <p>担当：{representative}</p>}
                {email && <p>Eメールアドレス：{email}</p>}
              </div>
            </section>
