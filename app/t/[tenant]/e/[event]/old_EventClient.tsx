...
export default function EventClient() {
  ...
  // 色定義: [ライトモード(新しい色)] : [ダークモード(今のままの色)]
  const theme = {
    bg: isLight ? "bg-slate-50" : "bg-[#080a14]",
    text: isLight ? "text-slate-800" : "text-white",
    subText: isLight ? "text-slate-500" : "text-slate-400",
    // ヘッダーのタグ（Official Eventなど）の背景
    headerTagBg: isLight ? "bg-white/80 border-slate-200 shadow-sm" : "bg-[#1A1D2D]/80 border-white/10 shadow-lg",
    // メインのガラス風カード背景
    cardBg: isLight ? "bg-white/90 border-slate-200 shadow-xl ring-slate-200" : "bg-[#131625]/70 border-white/10 shadow-2xl ring-white/5",
    // 内側のカード（講師プロフィールなど）
    innerCardBg: isLight ? "bg-slate-50 border-slate-200" : "bg-[#1A1D2D] border-white/5",
    // コンテンツボックス（地図など）
    contentBoxBg: isLight ? "bg-slate-100 border-slate-200" : "bg-slate-900/30 border-white/5",
    // 右カラムの背景
    rightColBg: isLight ? "bg-slate-100/50" : "bg-black/20",
    // フォームの背景
    formBg: isLight ? "bg-white/80 border-slate-200" : "bg-slate-900/80 border-slate-700/50",
    // 区切り線
    divider: isLight ? "border-slate-200" : "border-white/5",
  };
